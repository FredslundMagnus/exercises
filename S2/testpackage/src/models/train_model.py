...
def load(train_data: str, train_label: str, test_data: str, test_label: str) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
    return torch.load(join("data", "processed", train_data)), torch.load(join("data", "processed", train_label)), torch.load(join("data", "processed", test_data)), torch.load(join("data", "processed", test_label))
...
def train(lr, epochs):
    ...
    train_data, train_label, test_data, test_label = load("train_data.tensor", "train_labels.tensor", "test_data.tensor", "test_labels.tensor")
